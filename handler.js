...
export async function handler(chatUpdate) {
  ...
  /* Creditos a Otosaka (https://wa.me/56965960343) */
    ...
    case 'remove':
      if (chat.welcome && !chat?.isBanned) {
        ...
        for (const user of participants) {
          let pp = 'https://raw.githubusercontent.com/KillzN/Candy/master/src/avatar_contact.png';
          try {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;kimm 👑;;;\nFN:kimm 👑\nORG:kimm 👑\nTITLE:\nitem1.TEL;waid=56965960343:+51 992 621 601\nitem1.X-ABLabel:kimm 👑\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:kimm 👑\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, { contacts: { displayName: 'kimm 👑', contacts: [{ vcard }] } }, { quoted: callmsg });
